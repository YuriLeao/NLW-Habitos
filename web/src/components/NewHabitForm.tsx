...
export function NewhabitForm() {
    ...
    function handleWeekDay(weekDay: number) {
        if (weekDays.includes(weekDay)) {
            setWeekDays(weekDays.filter(d => d !== weekDay));
        } else {
    ...
    return (
        <form
                    ...
                    availableWeekDays.map((weekDay, index) => {
                        return (
                            <CheckBox.Root
                                ...
                                <span className="text-white leading-tight focus:outline-none focus:ring-2 focus:ring-violet-600 focus:ring-offset-2 focus:ring-offset-background">
                                    {weekDay}
                                </span>
                            </CheckBox.Root>
                        )
            ...
            <button
                type="submit"
                className="mt-6 rounded-lg p-4 flex items-center justify-center gap-3 font-semibold bg-green-600 hover:bg-green-500 transition-colorsfocus:outline-none focus:ring-2 focus:ring-green-600 focus:ring-offset-2 focus:ring-offset-zinc-900">
